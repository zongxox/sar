...
public class ExcelExportUtil0205 {
     ...
     */
    public static byte[] buildApplyDetailExcel(List<OrderShipmentDon0205Res> rows, boolean isXls) throws Exception {
        ...
        try (
            ...
            createTextCell(header, c++, "id", leftStyle);//0開始 往下就自增1
            createTextCell(header, c++, "訂單編號", leftStyle);//12
            createTextCell(header, c++, "客戶姓名", leftStyle);//1
            createTextCell(header, c++, "商品名稱", leftStyle);//2
            createTextCell(header, c++, "出貨數量", leftStyle);//3
            createTextCell(header, c++, "總金額", leftStyle);//4
            createTextCell(header, c++, "出貨地址", leftStyle);//5
            createTextCell(header, c++, "出貨狀態", leftStyle);//6
            createTextCell(header, c++, "出貨時間", leftStyle);//7
            createTextCell(header, c++, "建立時間", leftStyle);//8
            ...
            for (OrderShipmentDon0205Res res : rows) {
                Row row = sheet.createRow(r++);//在分頁1裡，建立 那一行(橫)欄位，順序要跟資料一致
                int col = 0; //第一行的第一個欄位開始

                createTextCell(row, col++, n(res.getId()), leftStyle);//0

                createTextCell(row, col++, n(res.getOrderNo()), leftStyle);

                createTextCell(row, col++, n(res.getCustomerName()), leftStyle);

                createTextCell(row, col++, n(res.getProductName()), leftStyle);

                createTextCell(row, col++, n(res.getQuantity()), leftStyle);

                createTextCell(row, col++, n(res.getTotalPrice()), leftStyle);

                createTextCell(row, col++, n(res.getShippingAddress()), leftStyle);

                createTextCell(row, col++, res.getStatus(), dateStyle);

                createDateCell(row, col++, res.getShippedAt(), dateStyle);
