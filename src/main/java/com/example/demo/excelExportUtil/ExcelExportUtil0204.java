...
public class ExcelExportUtil0204 {
     ...
     */
    public static byte[] buildApplyDetailExcel(List<TaskScheduleDon0204Res> rows, boolean isXls) throws Exception {
        ...
        try (
            ...
            createTextCell(header, c++, "活動標題", leftStyle);//12
            createTextCell(header, c++, "狀態", leftStyle);//1
            createTextCell(header, c++, "活動類型", leftStyle);//2
            createTextCell(header, c++, "金額", leftStyle);//3
            createTextCell(header, c++, "優先順序", leftStyle);//4
            createTextCell(header, c++, "備註說明", leftStyle);//5
            createTextCell(header, c++, "地點", leftStyle);//6
            createTextCell(header, c++, "開始時間", leftStyle);//7
            createTextCell(header, c++, "結束時間", leftStyle);//8
            ...
            for (TaskScheduleDon0204Res res : rows) {
                Row row = sheet.createRow(r++);//在分頁1裡，建立 那一行(橫)欄位，順序要跟資料一致
                int col = 0; //第一行的第一個欄位開始

                createTextCell(row, col++, n(res.getId()), leftStyle);//0

                createTextCell(row, col++, n(res.getTitle()), leftStyle);

                createTextCell(row, col++, n(res.getStatus()), leftStyle);

                createTextCell(row, col++, n(res.getType()), leftStyle);

                createTextCell(row, col++, n(res.getAmount()), leftStyle);

                createTextCell(row, col++, n(res.getPriority()), leftStyle);

                createTextCell(row, col++, n(res.getRemark()), leftStyle);

                createTextCell(row, col++, res.getLocation(), dateStyle);

                createDateCell(row, col++, res.getStartTime(), dateStyle);

                createDateCell(row, col++, res.getEndTime(), dateStyle);
