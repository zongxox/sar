...
public class ExcelExportUtil0130 {
     ...
     */
    public static byte[] buildApplyDetailExcel(List<ArticleDon0130Res> rows, boolean isXls) throws Exception {
        ...
        try (
            ...
            createTextCell(header, c++, "文章ID", leftStyle);//0開始 往下就自增1
            createTextCell(header, c++, "文章標題", leftStyle);//12
            createTextCell(header, c++, "文章內容", leftStyle);//1
            createTextCell(header, c++, "文章摘要", leftStyle);//2
            createTextCell(header, c++, "作者", leftStyle);//3
            createTextCell(header, c++, "文章分類", leftStyle);//4
            createTextCell(header, c++, "文章狀態", leftStyle);//5
            createTextCell(header, c++, "瀏覽次數", leftStyle);//6
            ...
            for (ArticleDon0130Res dto : rows) {
                ...
                // 2 評估日期（目前當字串處理）
                createTextCell(row, col++, n(dto.getTitle()), leftStyle);

                // 3 公司代號
                createTextCell(row, col++, n(dto.getContent()), leftStyle);

                // 4 檔案類型
                createTextCell(row, col++, n(dto.getSummary()), leftStyle);

                // 5 上傳檔案存放路徑
                createTextCell(row, col++, n(dto.getAuthor()), leftStyle);

                // 6 上傳檔案檔名
                createTextCell(row, col++, n(dto.getCategory()), leftStyle);

                // 7 上傳檔案檔名（原檔名）
                createTextCell(row, col++, n(dto.getStatus()), leftStyle);

                // 8 上傳檔案大小（KB）
                createTextCell(row, col++, n(dto.getViews()), leftStyle);

                // 9 上傳檔案帳號
                createDateCell(row, col++, dto.getCreatedAt(), dateStyle);

                // 10 上傳檔案時間（Excel 真日期）
                createDateCell(row, col++, dto.getUpdatedAt(), dateStyle);
